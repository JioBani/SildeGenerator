...
Harness 경계는 프론트엔드 변경을 막는 경계가 아니다. 사용자에게 새로운 연출의 선택·설정·진행 상태·결과 또는 비교 정보를 제공해야 하면 `web/`과 `server/`의 공개 API를 함께 수정한다. 사용자 기능을 연출 코드에 억지로 숨기거나 UI 변경을 피하기 위해 기능을 축소하지 않는다.
